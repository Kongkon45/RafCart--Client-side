            ...
            <div className="slider-container">
                <Slider {...settings}>
                    {data?.data?.map((product: any) => {
                        // console.log("working", product?.image)
                        ...
                        return <div key={product._id} className="border bg-white rounded-lg shadow-md pt-2 pb-5  text-center">
                            ...
                            <div className="flex justify-between items-center mx-6 mt-2">
                                <button className="text-sm font-semibold hover:bg-[#fd3d57] hover:text-white border border-[#fd3d57] rounded-lg text-[#fd3d57] py-1 px-2 transition-all ease-in-out duration-700"><Link href={`/products/${product?._id}`}>View Details</Link></button>
