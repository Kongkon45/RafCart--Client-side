            ...
            <form onSubmit={handleSubmit(onSubmit)}>
                <div className='lg:flex w-full mt-6 lg:px-0 px-10'>
                    <div className='lg:w-2/4 w-full mx-auto'>
                        <h2 className='w-full lg:text-xl text-md font-semibold rounded-lg text-center bg-[#e9e4e4] text-black pl-10 py-1 '>Billing Details</h2>
                        <div className='border-2 rounded-lg p-6 my-6'>
                            ...
                            <div className='flex gap-10 my-4'>
                                <div className='w-1/2'>
                                    ...
                                    <select className='w-full text-sm border-2 hover:border-[#fd3d57] rounded-md py-1 px-2' {...register("city", { required: true })}>
                                        <option value="">Select Town/City</option>
                                        {districts.map((district, index) => (
                                            <option key={index} value={district}>{district}</option>
                                        ))}
                                    </select>
                                    {errors.city && <span className='text-[#fd3d57] text-sm'>This field is required</span>}
                                </div>
                                <div className='w-1/2'>
                                    <label className='text-md font-bold' htmlFor="zipCode">Zip Code : <span className='text-md text-[#fd3d57]'>*</span></label> <br />
                                    <input className='w-full text-sm border-2 hover:border-[#fd3d57] rounded-md py-1 px-2 ' placeholder='Enter zip code...' type="text" {...register("zipCode", { required: true })} />
                                    {errors.zipCode && <span className='text-[#fd3d57] text-sm'>This field is required</span>}
                                </div>
                            </div>
                        </div>
                    </div>
                    ...
                    <div className='lg:w-1/4 w-full mx-auto'>
                        <h2 className='w-full lg:text-xl text-md rounded-lg font-semibold text-center bg-[#e9e4e4] text-black pl-10 py-1 '>Your Order</h2>
                        <div className='mt-6 border-2 rounded-lg p-4'>
                            ...
                            <ul>
                                {
                                    productItem.length === 0 ? (
                                        <li className='lg:text-2xl text-xl font-bold text-center'>Cart Not Found</li>
                                    ) : (
                                        productItem?.map((product: any) => {
                                            return <li key={product._id} className='flex justify-between items-center'>
                                                ...
                                                <h5 className="lg:text-md text-sm font-semibold">{product.title}</h5>
                                                <p className="lg:text-md text-sm font-semibold">{product.price}</p>
                                                <p className="lg:text-md text-sm font-semibold">X{product.quantity}</p>
                                                <p className='lg:text-md text-sm font-bold'>{(product.quantity) * (product.price)}</p>
                                            </li>
                                        })
                                    )
                                }
                            ...
                            <div className='mt-6 mb-2'>
                                ...
                                <div className='flex items-center gap-2 mt-6'>
                                    <input className='lg:w-4 w-3 lg:h-4 h-3' type="checkbox" {...register("checkbox", {required : true})} />
                                    <label className='lg:text-md text-sm ' htmlFor="checkbox">Agree to our Terms & Conditons</label>
                                </div>
                                {errors.checkbox && <span className='text-[#fd3d57] text-sm'>This field is required</span>}
                                <button className='w-full text-md mt-6 bg-[#fd3d57] text-white py-2 px-4 rounded-lg hover:border-2 hover:bg-white hover:text-[#fd3d57] hover:border-[#fd3d57] transition-all duration-200 ease-in-out ' type="submit">PLACE ORDER</button>
                            </div>
                        </div>
                    </div>
                </div>
